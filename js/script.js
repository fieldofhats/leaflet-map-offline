document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add GeoTIFF overlay
    fetch('./imagery/lidar_wgs84.tiff')
        ...
        .then(arrayBuffer => {
            parseGeoraster(arrayBuffer).then(georasterData => {
                ...
                geotiffLayer.setZIndex(10); // Set zIndex to ensure it is above other layers
    ...
    // Add event listeners to toggle collapsed class on mouse enter/leave
    const baseMapsContainer = baseMapsControl.getContainer();
    baseMapsContainer.addEventListener('mouseenter', () => {
        baseMapsContainer.classList.remove('collapsed');
    });
    baseMapsContainer.addEventListener('mouseleave', () => {
        baseMapsContainer.classList.add('collapsed');
    });

    const overlaysContainer = overlaysControl.getContainer();
    overlaysContainer.addEventListener('mouseenter', () => {
        overlaysContainer.classList.remove('collapsed');
    });
    overlaysContainer.addEventListener('mouseleave', () => {
        overlaysContainer.classList.add('collapsed');
    });

    // Initially collapse the controls
    baseMapsContainer.classList.add('collapsed');
    overlaysContainer.classList.add('collapsed');
});